...
## MLIA 2D slice dataset layout
The MLIA setup auto-discovers data under `../../Data/Segmentation_data/`:
```
Data/Segmentation_data/
  Training/
    Brains/mri_<id>.mhd (+ .raw)
    Labels/seg_<id>.mhd (+ .raw)
  Testing/
    Brains/...
    Labels/...
```
Single-modality 2D slices are repeated to 4 channels and stacked to a small depth (`--pseudo_3d_depth`, default 16) so pretrained 3D Swin UNETR weights can be finetuned.
...
- Inference on MLIA test/val:  
  ```bash
  python test.py --data_dir=../../Data/Segmentation_data/ \
    --pretrained_dir=./pretrained_models/ --pretrained_model_name=<model.pt> \
    --roi_x=96 --roi_y=96 --roi_z=32 --pseudo_3d_depth=32 --exp_name=mlia_infer
  ```
...
```bash
python main.py --data_dir=../../Data/Segmentation_data/ --val_every=5 --noamp \
--roi_x=96 --roi_y=96 --roi_z=32 --pseudo_3d_depth=32 --in_channels=4 --spatial_dims=3 --use_checkpoint --feature_size=48
```
